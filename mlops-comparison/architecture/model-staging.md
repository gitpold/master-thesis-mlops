...
| **Decision** | **Option B:** Deploy Code |
|-------|-------|
| **Justification** | • provides a more stable and clearer process, that aligns with the current staging concept for application development <br>• as the focus is currently on using LLMs for example in RAG systems and no own training of expensive models planned, rebuilding/retraining models on each stage has no resource/cost issue |
| **Decided by** |  |
